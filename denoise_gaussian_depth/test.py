...
parser.add_argument("--model", default="./checkpoint/model_nyu2_net20060_50.pth", type=str, help="model path")
parser.add_argument("--dataset", default="./depth", type=str, help="dataset name, Default: KODAK")
parser.add_argument("--save", default="./results", type=str, help="savepath, Default: results")
parser.add_argument("--noise_sigma", default=15, type=int, help="standard deviation of the Gaussian noise, Default: 25")
...
with torch.no_grad():
    for image_name in image_list:
        ...
        im_gt = Variable(im_n)
        noise = np.random.normal(size=im_gt.shape) * opt.noise_sigma/255.0
        noise = torch.from_numpy(noise).float()
        im_input=im_gt + noise
