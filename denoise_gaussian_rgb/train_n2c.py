...
parser.add_argument("--noise_sigma", default=50, type=str, help="standard deviation of the Gaussian noise (default: 50)")
...
def main():
    ...
    file = open('./checksample/mse_n2c_'+str(opt.noise_sigma)+str(opt.nEpochs)+'.txt','w')
    for mse in MSE:
        file.write(mse+'\n')
    file.close()
...
def train(training_data_loader, G_optimizer, model, criterion, epoch):
    ...
    for iteration, batch in enumerate(training_data_loader, 1):
        ...
        # rng_stddev = np.random.uniform(0.01, opt.noise/255.0,[1,1,1])
...
def save_checkpoint(model, epoch):
    model_out_path = "./checkpoint/model_denoise_n2c"+str(opt.noise_sigma)+str(opt.nEpochs)+".pth"
