...
parser.add_argument("--noise_lambda", default=30, type=int, help="maximum event count for poisson noise, Default: 30")
...
def main():
    ...
    file = open('./checksample/mse_'+str(opt.noise_lambda)+'_'+str(opt.nEpochs)+'_'+str(opt.sigma)+'.txt','w')
    for mse in MSE:
        file.write(mse+'\n')
    file.close()

    file = open('./checksample/Gloss_'+str(opt.noise_lambda)+'_'+str(opt.nEpochs)+'_'+str(opt.sigma)+'.txt', 'w')
    for g in GLOSS:
        file.write(g + '\n')
    file.close()
    # psnr = eval_dep(model)
...
def save_checkpoint(model, discr, epoch):
    model_out_path = "checkpoint/" + "model_denoise_"+str(opt.noise_lambda)+"_"+str(opt.nEpochs)+"_"+str(opt.sigma)+".pth"
