...
parser.add_argument("--noise_lambda", default=30, type=int, help="standard deviation of the Gaussian noise (default: 30)")
...
def main():
    ...
    file = open('./checksample/mse_n2n_'+str(opt.noise_lambda)+str(opt.nEpochs)+'.txt','w')
    for mse in MSE:
        file.write(mse+'\n')
    file.close()
...
def train(training_data_loader, G_optimizer, model, criterion, epoch):
    ...
    for iteration, batch in enumerate(training_data_loader, 1):
        ...
        # rng_lambda1 = np.random.uniform(10, opt.noise_lambda, [1,1,1])
        # input=np.random.poisson(rng_lambda1*target.numpy())/rng_lambda1
        # rng_lambda2 = np.random.uniform(10, opt.noise_lambda, [1,1,1])
        # target=np.random.poisson(rng_lambda2*target.numpy())/rng_lambda2
...
def save_checkpoint(model, epoch):
    model_out_path = "./checkpoint/model_unet_n2n"+str(opt.noise_lambda)+str(opt.nEpochs)+".pth"
